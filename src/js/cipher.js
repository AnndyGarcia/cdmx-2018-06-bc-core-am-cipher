  ...
  encode: (string, offset) => { // Funcion encode que codifica el texto
    ...
    for (let j = 0; j < string.length; j++) { // for que recorre el string
      if (string.charCodeAt(j) === 32) { // Identifica si hay espacios
        ...
        message.push((string.charCodeAt(j) - 97 + offset) % 26 + 97); // Convierte las letras del mensaje en codigos ASCII, aplica la formula y las almacena
        ...
        message.push((string.charCodeAt(j) - 65 + offset) % 26 + 65); // Convierte las letras del mensaje en codigo ASCII, aplica la formula y las almacen
        code.push(String.fromCharCode(message[j])); // Transforma los codigos ASCII en letras y las almacena
      }
    }
  ...
  decode: (string, offset) => { // Funcion que decodifica
    ...
    for (let j = 0; j < string.length; j++) { // for que recorre el string
      if (string.charCodeAt(j) === 32) { // Identifica espacios
        ...
        message.push((string.charCodeAt(j) - 122 - offset) % 26 + 122); // Convierte las letras del mensaje en codigo ASCII, aplica la formula y las almacena
        ...
        message.push((string.charCodeAt(j) - 90 - offset) % 26 + 90); // Convierte las letras del mensaje en codigo ASCII, aplica la formula y las almacena
        code.push(String.fromCharCode(message[j])); // Convierte los codigos ASCII en letras y las almacena
      }
    }
    let newCode = code.join(''); // Elimina las comas entre cada elemento del string code, y las guarda en un nuevo string
